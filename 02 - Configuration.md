...
In most instances where you would use a promise, you will also want to inject an
...
That will allow you to inject the `contentResource` service. All you need to do
in order to have a dependency injected is return a function with that dependency
...
You will likely want to add some caching so subsequent requests to your
...
The above code can also be rewritten like so and it would function the same way,
though perhaps slightly faster:
...
This demonstrates that you can return different types of objects
...
Here is a heavily commented version of the above function, which should help
you to understand what each line is doing:
